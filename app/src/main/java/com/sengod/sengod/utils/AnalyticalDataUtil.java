...
public class AnalyticalDataUtil {
    ...
    public static void analyData(Context context, byte[] value, AnalyticalDataUtilsInterface analyticalDataUtilsInterface) {
        ...
        try {
            ...
            //判断剩余数据
            if (len == 0) {
                len = Integer.parseInt(tempResult[0], 16);
            }
            ...
            if (len + 3 == result.length) {
                ...
                if (crc.equals(crcresult)) {
                    ...
                    switch (result[1]) {
                        ...
                        case "92":
                        case "93":
                            String state = String.valueOf(parseInt(result[2], 16));
                            switch (state) {
                                case "1":
                        ...
                        case "1":
                            ...
                            //出厂时间
                            listenerResult[0] = String.format(context.getResources().getString(R.string.activity_data_record_factory_time_data), String.valueOf(parseInt(result[3] + result[2], 16)), String.valueOf(parseInt(result[4], 16)), String.valueOf(parseInt(result[5], 16)));
                            //版本号
                            listenerResult[1] = String.format(context.getResources().getString(R.string.activity_data_record_factory_version_data), String.valueOf(parseInt(result[7] + result[6], 16)), String.valueOf(parseInt(result[9] + result[8], 16)));
                            //充电总次数
                            listenerResult[2] = String.format(context.getResources().getString(R.string.activity_data_record_total_charging_times_data), parseInt(result[29] + result[28], 16));
                            //使用时间
                            Integer tminutes = parseInt(result[13] + result[12] + result[11] + result[10], 16);
                            ...
                            //行走时间
                            tminutes = parseInt(result[17] + result[16] + result[15] + result[14], 16);
                            hour = tminutes / 60;
                            min = tminutes % 60;
                            listenerResult[4] = String.format(context.getResources().getString(R.string.activity_data_record_walk_time_data), hour, min);
                            //载物时间
                            tminutes = parseInt(result[21] + result[20] + result[19] + result[18], 16);
                            hour = tminutes / 60;
                            min = tminutes % 60;
                            listenerResult[5] = String.format(context.getResources().getString(R.string.activity_data_record_carry_time_data), hour, min);
                            //顶升次数
                            listenerResult[6] = String.format(context.getResources().getString(R.string.activity_data_record_lift_time_data), parseInt(result[21] + result[20] + result[19] + result[18], 16));
                            break;
                        //读参数设置
                        case "3":
                            listenerResult = new String[11];
                            //空载加减速
                            listenerResult[0] = String.valueOf(parseInt(result[3] + result[2], 16));
                            listenerResult[1] = String.valueOf(parseInt(result[5] + result[4], 16));

                            //带载加减速
                            listenerResult[2] = String.valueOf(parseInt(result[7] + result[6], 16));
                            listenerResult[3] = String.valueOf(parseInt(result[9] + result[8], 16));

                            //顶升调度
                            listenerResult[4] = String.valueOf(parseInt(result[11] + result[10], 16));

                            //顶升速度
                            listenerResult[5] = String.valueOf(parseInt(result[13] + result[12], 16));

                            //减速距离
                            listenerResult[6] = String.valueOf(parseInt(result[15] + result[14], 16));

                            //减速加速度
                            listenerResult[7] = String.valueOf(parseInt(result[17] + result[16], 16));

                            //旋转速度
                            listenerResult[8] = String.valueOf(parseInt(result[19] + result[18], 16));

                            //左轮直径
                            listenerResult[9] = String.valueOf(parseInt(result[23] + result[22], 16));

                            //右轮直径
                            listenerResult[10] = String.valueOf(parseInt(result[25] + result[24], 16));
                            break;
                        //读动态测试
                        case "5":
                            listenerResult = new String[17];
                            //压力波开关38
                            listenerResult[0] = String.valueOf(parseInt(result[32], 16));
                            listenerResult[1] = String.valueOf(parseInt(result[33], 16));
                            //急停
                            listenerResult[2] = String.valueOf(parseInt(result[34], 16));
                            //充电插座
                            listenerResult[3] = String.valueOf(parseInt(result[37], 16));
                            listenerResult[4] = String.valueOf(parseInt(result[38], 16));
                            listenerResult[5] = String.valueOf(parseInt(result[39], 16));
                            //电池电压
                            listenerResult[6] = String.format(context.getResources().getString(R.string.activity_dynamic_test_battery_voltage_data), String.valueOf(parseInt(result[3] + result[2], 16)));
                            //电池温度
                            listenerResult[7] = String.format(context.getResources().getString(R.string.activity_dynamic_test_battery_temperature_data), parseInt(result[5] + result[4], 16));
                            //障碍物距离监测
                            listenerResult[8] = String.format(context.getResources().getString(R.string.activity_dynamic_test_obstacle_distance_data), parseInt(result[15] + result[14], 16));
                            //行走伺服
                            listenerResult[9] = String.valueOf(parseInt(result[16], 16));
                            listenerResult[10] = String.valueOf(parseInt(result[20], 16));
                            //行走伺服工作电流
                            listenerResult[11] = String.format(context.getResources().getString(R.string.activity_dynamic_test_walking_current_data), String.valueOf(parseInt(result[19] + result[18], 16)));
                            listenerResult[12] = String.format(context.getResources().getString(R.string.activity_dynamic_test_walking_current_data), String.valueOf(parseInt(result[23] + result[22], 16)));
                            //顶升伺服
                            listenerResult[13] = String.valueOf(parseInt(result[24], 16));
                            //顶升伺服工作电流
                            listenerResult[14] = String.format(context.getResources().getString(R.string.activity_dynamic_test_jacking_current_data), String.valueOf(parseInt(result[27] + result[26], 16)));
                            //旋转伺服
                            listenerResult[15] = String.valueOf(parseInt(result[28], 16));
                            //旋转伺服工作电流
                            listenerResult[16] = String.format(context.getResources().getString(R.string.activity_dynamic_test_rotate_current_data), String.valueOf(parseInt(result[31] + result[30], 16)));
                            break;
                        //读地址二维码
                        case "6":
                            //地址二维码数据
                            ...
                            if (result[3].startsWith("F") || result[3].startsWith("f")) {
                                listenerResult[2] = String.valueOf(parseInt(result[3] + result[2], 16) - 65536);
                            } else {
                                listenerResult[2] = String.valueOf(parseInt(result[3] + result[2], 16));
                            }
                            if (result[5].startsWith("F") || result[5].startsWith("f")) {
                                listenerResult[3] = String.valueOf(parseInt(result[5] + result[4], 16) - 65536);
                            } else {
                                listenerResult[3] = String.valueOf(parseInt(result[5] + result[4], 16));
                            }

                            LogUtils.i("TAG", "angle:" + String.format("%.2f", parseInt(result[7] + result[6], 16) / 1000.0 * 180.0 / 3.1415926));
                            if (result[7].startsWith("F") || result[7].startsWith("f")) {
                                LogUtils.i("TAG", "angle parseInt:" + (parseInt(result[7] + result[6], 16) - 65536));
                                listenerResult[4] = String.format("%.2f", (parseInt(result[7] + result[6], 16) - 65536) / 1000.0 * 180.0 / 3.1415926);
                            } else {
                                listenerResult[4] = String.format("%.2f", parseInt(result[7] + result[6], 16) / 1000.0 * 180.0 / 3.1415926);
                            }
                            break;
                        //读货架二维码
                        case "7":
                            ...
                            String tempStr = "";
                            for (int j = 8; j < result.length - 2; j++) {
                                tempStr += completeToOneByte(result[j]);
                            }
                            listenerResult[0] = hexStringToString(tempStr);

                            if (result[3].startsWith("F") || result[3].startsWith("f")) {
                                listenerResult[1] = String.valueOf(parseInt(result[3] + result[2], 16) - 65536);
                            } else {
                                listenerResult[1] = String.valueOf(parseInt(result[3] + result[2], 16));
                            }

                            if (result[5].startsWith("F") || result[5].startsWith("f")) {
                                listenerResult[2] = String.valueOf(parseInt(result[5] + result[4], 16) - 65536);
                            } else {
                                listenerResult[2] = String.valueOf(parseInt(result[5] + result[4], 16));
                            }

                            if (result[7].startsWith("F") || result[7].startsWith("f")) {
                                LogUtils.i("TAG", "angle parseInt:" + (parseInt(result[7] + result[6], 16) - 65536));
                                listenerResult[3] = String.format("%.2f", (parseInt(result[7] + result[6], 16) - 65536) / 1000.0 * 180.0 / 3.1415926);
                            } else {
                                listenerResult[3] = String.format("%.2f", parseInt(result[7] + result[6], 16) / 1000.0 * 180.0 / 3.1415926);
                            }
                            break;
                        //状态查询
                        case "13":
    ...
    public static void analyDataOld(Context context, byte[] value, AnalyticalDataUtilsInterface analyticalDataUtilsInterface) {
        //判断是否是同一批数据
        if (System.currentTimeMillis() - startTime > 2000) {
            tempResult = new String[19];
            result = new String[]{};
